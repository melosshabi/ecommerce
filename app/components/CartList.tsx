        ...
        return (
          <div className="flex flex-col my-2 border-b border-black py-2 xl:flex-row xl:justify-around" key={index}>
            <div className="flex justify-center items-start h-fit">
              <Image className="w-[170px]" src={item.productImage as string} width={200} height={200} alt="Cart item image"/>
            </div>
            ...
            <div className="flex justify-between my-8 xl:w-[40dvw]">
              <button className="p-2 rounded-md mx-4 transition-all duration-200 hover:bg-transparentBlack" onClick={() => removeFromCart(session.data?.user.userDocId ? session.data?.user.userDocId : undefined, item._id)}>

...
export default function ProfilePageSidebar({activePage} : ProfilePageSidebar) {
    ...
    useEffect(() => {
        document.querySelector(`.${activePage}-div`)?.classList.add('bg-transparentBlack')
        document.querySelector(`.${activePage}-div svg`)?.classList.add('fill-white')
        return () => {
            document.querySelector(`.${activePage}-div`)?.classList.remove('bg-transparentBlack')
            document.querySelector(`.${activePage}-div svg`)?.classList.remove('fill-white')
        }
    }, [activePage])

return (
    <div className="flex justify-center items-center mt-[12dvh] mb-6 bg-white shadow-[0_0_5px_black] w-[90dvw] m-auto rounded-lg">
        <div className='flex w-full justify-between'>
            {
                session.status === "authenticated" &&
                <Link href="/userProfile/account" className="transition-all duration-200 hover:bg-transparentBlack">
                    <div className="account-div p-3">
                        <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M304 128a80 80 0 1 0 -160 0 80 80 0 1 0 160 0zM96 128a128 128 0 1 1 256 0A128 128 0 1 1 96 128zM49.3 464H398.7c-8.9-63.3-63.3-112-129-112H178.3c-65.7 0-120.1 48.7-129 112zM0 482.3C0 383.8 79.8 304 178.3 304h91.4C368.2 304 448 383.8 448 482.3c0 16.4-13.3 29.7-29.7 29.7H29.7C13.3 512 0 498.7 0 482.3z"/></svg>
                            <span className="hidden" >Account Information</span>
                    </div>
                </Link>
            }
            <Link href="/userProfile/cart">
                <div className="cart-div p-3 transition-all duration-200 hover:bg-transparentBlack">
            ...
            <Link href="/userProfile/wishlist">
                <div className="wishlist-div p-3 transition-all duration-200 hover:bg-transparentBlack">
            ...
            {session.status === 'authenticated' && 
                <Link href="/userProfile/userProducts">
                    <div className="userProducts-div p-3 transition-all duration-200 hover:bg-transparentBlack">
                    <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M50.7 58.5L0 160H208V32H93.7C75.5 32 58.9 42.3 50.7 58.5zM240 160H448L397.3 58.5C389.1 42.3 372.5 32 354.3 32H240V160zm208 32H0V416c0 35.3 28.7 64 64 64H384c35.3 0 64-28.7 64-64V192z"/></svg>
                        <span className="hidden" >My Products</span>
                    </div>
                </Link>
            } 
            <Link href="/userProfile/userOrders">
                <div className='userOrders-div p-3 transition-all duration-200 hover:bg-transparentBlack'>

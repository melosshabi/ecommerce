...
export default function UserProductsList() {
  ...
  useEffect(() => {
    ...
    async function fetchUserProducts(){
      try{
        const res = await fetch('http://localhost:3000/api/getUserProducts')
              ...
              <div className="user-product" key={index}>
                ...
                <div className="user-product-options">
                  ...
                  <div className="user-product-options-btns">
                    <button className="user-product-btns reduce-quantity-btn">
                      <Image className='quantity-icons' src={minus} width={50} height={50} alt="Minus icon"/>
                    </button>
                    
                    <input value={product.quantity} className='quantity-input'/>

                    <button className="user-product-btns increase quantity-btn">
                      <Image className='quantity-icons' src={plus} width={50} height={50} alt="Plus icon"/>
                    </button>
                  </div>
                </div>
                <button className='user-product-btns user-product-remove-btn'>
                ...
                <span className='date-posted'>Posted: {year} {month} {day} {hours}:{minutes}</span>
              </div>
            )
          })
        }

...
export default function Wishlist({productsArray, userDocId}:WishlistProps) {
  ...
  return (
    <div className={`h-[80dvh] w-[95dvw] m-auto shadow-[0_0_5px_black] rounded-lg p-2 overflow-y-scroll lg:w-[60dvw] lg:m-0 ${wishlistItems.length === 0 ? 'flex justify-center items-center' : ''}`}>
    ...
    }
    {
        ...
        return (
            <div className="flex flex-col items-center my-4 py-4 border-b border-black md:flex-row md:justify-around lg:justify-start" key={index}>
                <div >
                <Image className="" src={item.pictures[0] as string} width={200} height={200} alt="Wishlist item image"/>
            </div>
            ...
            <div className='text-center lg:m-auto'>
                ...
                <div className="flex justify-center my-6">
                    ...
                    <button className="p-2 rounded-md mx-4 transition-all duration-200 hover:bg-transparentBlack" title="Remove from wishlist" onClick={() => removeFromWishlist(userDocId, item._id)}>
